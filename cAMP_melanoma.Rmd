...
plots$goi_subsets$tsoi_sigs_PC1PC2 <- biplot_diff(pca$goi_subsets$tsoi_sigs, x="PC1", y="PC2") + 
...
for (tf in 1:length(anno$tf_targets)){
  ...
  pca[["goi_subsets"]][["tf_targets"]][[tf_name]] <- pca(data[["goi_subsets"]][["tf_targets"]][[tf_name]], metadata = metadata)
  
  plots[["goi_subsets"]][["tf_targets"]][[tf_name]] <- biplot_diff(pca[["goi_subsets"]][["tf_targets"]][[tf_name]], x="PC1", y="PC2") + 
...
data$mean_z <- read_csv(file.path("output", dataset_name, "mean_zscores", "all_genesets_z.csv")) |> 
  rename("...1"="sample")

pca$goi_subsets$tsoi_sigs$metadata <- left_join(pca$goi_subsets$tsoi_sigs$metadata, data$mean_z, by="sample")

eigen <- list()
eigen$corr_tftargets <- eigencorplot(pca$goi_subsets$tsoi_sigs, 
                                    metavars = pca$goi_subsets$tsoi_sigs$metadata |> dplyr::select(contains("target")) |> colnames(),
...
eigen$corr_tfs <- eigencorplot(pca$goi_subsets$tsoi_sigs, 
                                    metavars = pca$goi_subsets$tsoi_sigs$metadata |>
                                               dplyr::select(!contains(c("targets","diffstate","signature", "GO_", "bp_", "cell_line", "sample", "source"))) |> 
                                               colnames(),
                                    components = getComponents(pca$goi_subsets$tsoi_sigs, 1:4),
                                    col = c('white', 'cornsilk1', 'gold', 'forestgreen', 'darkgreen'),
                                    cexCorval = 0.75,
...
lr$plot_list <- c("creb_family_tfs", "creb_binding_cofactors", "creb_activity_markers", "ap1_family_tfs", #grouped GOIs
                  "AXL", "NGFR", "SOX10", "MITF", #basic melanoma diff state markers
                  "NR4A1", "NR4A2", "NR4A3", # marker of creb activity 
                  #"ATF1", "ATF4", "CREB1", "CREB3", "CREB5", "CREB3L1", "CREB3L2", "CREB3L3", "CREB3L4", "CREBL2", "CREBZF", "CREM", # creb family tfs
                  #"CRTC2", "CRTC3", "EID1", "EP300", "PPARGC1A", # creb binding cofactors
                  #"FOS", "FOSL1", "FOSL2", "JUN", "JUNB", # ap1 family tfs
                  "creb1_targets", "crebbp_targets", "crem_targets", "ep300_targets", "fos_targets", "fosl1_targets", "jun_targets", "jund_targets",
                  "mitf_targets", "nf1_targets", "nr4a2_targets", "ppargc1a_targets" #tf targets mean z scores
                  )
...
lr$plot <- ggcorrplot(lr$plot_r) + ggtitle("Pearson's r for transcription factor targets vs.\ndifferentiation state signatures (mean z-scores)")
